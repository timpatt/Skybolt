...
void HudSystem::updateState()
{
	...
	if (mEntity && mEnabled)
	{
		...
		Motion* motion = mEntity->getFirstComponent<Motion>().get();
		...
		if (node && motion)
		{
			...
			const Vector3 uprightLocalVel = glm::inverse(uprightYawFrameOrientation) * motion->linearVelocity;
			...
			mHud->drawText(glm::vec2(-0.4, 0.35), toIntString(glm::length(motion->linearVelocity) * metersPerSecToKnots));
