...
#include "SkyboltVisOsg/Renderable/BuildingsBatch.h"
#include "SkyboltVisOsg/Renderable/RoadsBatch.h"
#include "SkyboltVisOsg/Renderable/RunwaysBatch.h"
#include "SkyboltVisOsg/Renderable/Water/LakesBatch.h"
...
public:
	...
	std::unique_ptr<LoadedVisObjects> loadVisObjects(const std::vector<mapfeatures::FeaturePtr>& features, const sim::LatLon& latLonOrigin, double planetRadius) const
	{
		...
		LlaToNedConverter converter(latLonOrigin, planetRadius);
		...
		for (const mapfeatures::FeaturePtr& feature : features)
		{
			...
			case mapfeatures::FeatureRoad:
			{
				...
				{
					osg::Vec3f pos = math::toOsgVec3f(converter.latLonAltToCartesianNed(srcRoad.points[j]));
				...
				for (int i = 0; i < 2; ++i)
				{
					...
					{
						road.endControlPoints[i] = math::toOsgVec3f(converter.latLonAltToCartesianNed(srcRoad.endControlPoints[i]));
					}
				}
			...
			case mapfeatures::FeatureBuilding:
			{
				...
				{
					osg::Vec3f pos = math::toOsgVec3f(converter.latLonAltToCartesianNed(srcBuilding.points[j]));
			...
			case mapfeatures::FeatureWater:
			{
				...
				{
					osg::Vec3f pos = math::toOsgVec3f(converter.latLonAltToCartesianNed(srcWater.points[j]));
			...
			case mapfeatures::FeatureAirport:
			{
				...
				{
					Runway runway;
					runway.startPoint = math::toOsgVec3f(converter.latLonAltToCartesianNed(toLatLonAlt(srcRunway.start, srcAirport.altitude)));
					runway.endPoint = math::toOsgVec3f(converter.latLonAltToCartesianNed(toLatLonAlt(srcRunway.end, srcAirport.altitude)));
				...
				if (0)
				{
					for (const LatLonPoints& polygon : srcAirport.areaPolygons)
					{
						...
						{
							region.points.push_back(math::toOsgVec3f(converter.latLonAltToCartesianNed(toLatLonAlt(polygon[j], srcAirport.altitude))));
						}
