...
static void setupTerrainStateSet(osg::StateSet& ss, const TerrainConfig& config)
{
	...
	ss.addUniform(new osg::Uniform("heightScale", config.elevationRerange.x * 65535.f));
	ss.addUniform(new osg::Uniform("heightOffset", config.elevationRerange.y));
...
Terrain::Terrain(const TerrainConfig& config)
{
	...
	{
		osg::Vec3d tilePosition = llaToGeocentric(math::toOsgVec2d(planetTile->latLonBounds.center()), 0, planetTile->planetRadius);
