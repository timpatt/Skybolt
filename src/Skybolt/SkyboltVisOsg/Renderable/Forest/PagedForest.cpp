...
public:
	PageGeneratorTask(const osg::ref_ptr<osg::Image>& attributeMap, const ShaderPrograms* programs,
		const ElevationProviderPtr& elevationProvider, const Box2d& bounds, Vec2Transform converter, float visRangeWorldUnits, const osg::Vec2f pageSize) :
	...
	osg::Group* run(const osg::Vec2i& pageId) const
	{
		...
		Box2d worldBounds;
		worldBounds.minimum = mBounds.minimum + glm::dvec2(pageId.x() * mPageSize.x(), pageId.y() * mPageSize.y());
		worldBounds.maximum = worldBounds.minimum + glm::dvec2(mPageSize.x(), mPageSize.y());
		worldBounds.maximum = glm::dvec2(std::min(worldBounds.maximum.x, mBounds.maximum.x), std::min(worldBounds.maximum.y, mBounds.maximum.y));

		Box2d imageBounds = getSubImageBounds(mBounds, worldBounds, image.width, image.height);
		...
		const glm::dvec2 worldBoundsSize = worldBounds.size();
		const osg::Vec2f worldBoundsSizeOsg(worldBoundsSize.x, worldBoundsSize.y);
		BillboardForest::addGeodes(*group, trees, mPrograms->getRequiredProgram("treeSideBillboard"), mPrograms->getRequiredProgram("treeTopBillboard"), mVisRangeWorldUnits, worldBoundsSizeOsg);
		return group;
	}
...
PagedForest::PagedForest(px_sched::Scheduler& scheduler, const osg::ref_ptr<osg::Image>& attributeMap, const ShaderPrograms* programs,
						 const ElevationProviderPtr& elevationProvider, const Box2d& bounds, const glm::dvec2& maxPageSize,
						 Vec2Transform converter, float visRangeWorldUnits) :
	mScheduler(scheduler),
	mBounds(bounds),
	mVisibilityRange(maxPageSize.x, maxPageSize.y)
{
	glm::dvec2 pageCountF = math::componentWiseDivide(bounds.size(), maxPageSize);
	mPageCount.x() = (int)std::ceil(pageCountF.x);
	mPageCount.y() = (int)std::ceil(pageCountF.y);
	assert(mPageCount.x() > 0);
	assert(mPageCount.y() > 0);

	mPageSize = math::componentWiseDivide(bounds.size(), glm::dvec2(mPageCount.x(), mPageCount.y()));

	mModelMatrixUniform = new osg::Uniform("modelMatrix", osg::Matrixf());
	mTransform->getOrCreateStateSet()->addUniform(mModelMatrixUniform);

	mPageGeneratorTask.reset(new PageGeneratorTask(attributeMap, programs, elevationProvider, bounds, converter, visRangeWorldUnits, osg::Vec2f(mPageSize.x, mPageSize.y)));
}
