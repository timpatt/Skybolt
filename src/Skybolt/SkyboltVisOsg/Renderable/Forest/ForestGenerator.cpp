...
std::vector<BillboardForest::Tree> ForestGenerator::generate(const ElevationProvider& elevation, const AttributeImage& image, const Box2d& worldBounds, const Box2d& imageBounds)
{
	std::vector<BillboardForest::Tree> result;

	glm::dvec2 cellSize = worldBounds.maximum - worldBounds.minimum;
	cellSize.x /= imageBounds.maximum.y - imageBounds.minimum.y;
	cellSize.y /= imageBounds.maximum.x - imageBounds.minimum.x;

	glm::dvec2 offset(worldBounds.minimum.x - cellSize.x * imageBounds.minimum.y,
	...
	for (int y = yStart; y < yEnd; ++y)
	{
		for (int x = xStart; x < xEnd; ++x)
		{
			...
			if (it != image.attributes.end())
			{
				...
				float xMin = std::max(double(y), imageBounds.minimum.y) * cellSize.x + offset.x;
				float yMin = std::max(double(x), imageBounds.minimum.x) * cellSize.y + offset.y;
				float xMax = std::min(double(y + 1), imageBounds.maximum.y) * cellSize.x + offset.x;
				float yMax = std::min(double(x + 1), imageBounds.maximum.x) * cellSize.y + offset.y;
