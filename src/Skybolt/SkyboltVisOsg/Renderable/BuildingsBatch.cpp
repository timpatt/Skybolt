...
static osg::Geode* createBuildings(const Buildings& buildings, const BuildingTypes& buildingTypes)
{
	...
	for (size_t i = 0; i < buildings.size(); ++i)
	{
		...
		if (!buildingTypes.roofs.empty())
		{
			int roofTextureIndex = random.getInt(buildingTypes.facades.size(), buildingTypes.facades.size() + buildingTypes.roofs.size() - 1);
...
static osg::ref_ptr<osg::Texture2DArray> createTextureArray(const BuildingTypes& buildingTypes)
{
	//! Texture array containing facades, then roofs
	osg::ref_ptr<osg::Texture2DArray> texture = createTextureArray();

	int textureIndex = 0;
	for (const BuildingTypes::Facade& facade : buildingTypes.facades)
	{
		osg::Image* image = readImageWithCorrectOrientation(facade.albedoTextureFilename);
		image->setInternalTextureFormat(toSrgbInternalFormat(image->getInternalTextureFormat()));
		texture->setImage(textureIndex, image);
		textureIndex++;
	}
	
	for (const BuildingTypes::Roof& roof : buildingTypes.roofs)
	{
		osg::Image* image = readImageWithCorrectOrientation(roof.albedoTextureFilename);
		image->setInternalTextureFormat(toSrgbInternalFormat(image->getInternalTextureFormat()));
		texture->setImage(textureIndex, image);
		textureIndex++;
	}
...
BuildingsBatch::BuildingsBatch(const Buildings& buildings, const osg::ref_ptr<osg::Program>& program, const BuildingTypesPtr& buildingTypes)
{
	...
	geode->setStateSet(createStateSet(program, createTextureArray(*buildingTypes), mUniforms));
