...
#include "SkyboltVisOsg/earcutOsg.h"
...
void RoadsBatch::replaceAttribute(const Roads& roads, osg::Image& image, const Box2f& imageWorldBounds, int attributeToReplace, int attributeToReplaceWith)
{
	...
	glm::vec2 minimumBound = imageWorldBounds.minimum;
	glm::vec2 scale = glm::vec2(image.s(), image.t()) / (imageWorldBounds.maximum - minimumBound);
	...
	for (const Road& road : roads)
	{
		...
		{
			glm::vec2 p0 = math::toGlmVec2(toVec2f(road.points[i]));
			glm::vec2 p1 = math::toGlmVec2(toVec2f(road.points[i + 1]));
