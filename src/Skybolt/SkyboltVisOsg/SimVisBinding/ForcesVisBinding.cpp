...
#include <SkyboltVisOsg/Renderable/Arrows.h>
			...
			if (entity->isDynamicsEnabled())
			{
				...
				if (node && body)
				{
					...
					for (const sim::AppliedForce& force : forces)
					{
						...
						segment.start = math::toOsgVec3d(converter.convertPosition(start));
						segment.end = math::toOsgVec3d(converter.convertPosition(start + force.force * forceScaleMetersPerNewton));
