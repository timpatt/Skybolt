...
void SimVisSystem::updateState()
{
	...
	if (planetPose && prevPlanetPose)
	{
		...
		glm::vec3 originTranslation = mCoordinateConverter->convertPosition(originWorldSpaceUsingPrevPlanetPose);
		mScene->translateNoiseOrigin(math::toOsgVec3f(-originTranslation));
	}
