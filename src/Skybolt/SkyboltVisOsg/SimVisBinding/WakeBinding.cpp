...
#include <SkyboltVisOsg/Renderable/Water/WaterStateSet.h>
#include <assert.h>
	...
	for (const sim::EntityPtr& entity : mWorld->getEntities())
	{
		if (auto* component = entity->getFirstComponent<sim::ShipWakeComponent>().get())
		{
			...
			if (position)
			{
				...
				osg::Vec3d wakeDir = math::toOsgVec3d(converter.convertLocalPosition(dir));
				wakeDir.z() = 0;
				wakeDir.normalize();
				...
				osg::Vec3d startPoint = math::toOsgVec3d(converter.convertPosition(*position));
