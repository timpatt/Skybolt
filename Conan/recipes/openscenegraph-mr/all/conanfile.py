...
class OpenSceneGraphConanFile(ConanFile):
    ...
    def source(self):
        ...
        git.checkout(self.conan_data["sources"][self.version]["branch"])
        rmdir(self, self.source_folder + "/" + self.name + "/.git")
        copy(self, pattern="*", src=self.source_folder + "/" + self.name, dst=self.source_folder)
        rmdir(self, self.source_folder + "/" + self.name)
